<template>
  <h1 class="page-title font-bold">{{ t('menu.billing_info') }}</h1>
  <VaCard>
    <VaCardContent>
      ...
      <BillTable
        :items="items"
        :loading="adminBillsStore.loading"
...
import { Filters, Pagination } from '../../../data/pages/bills'
import { useAdminBillsStore } from '../../../stores/admin-bills'
...
const adminBillsStore = useAdminBillsStore()
...
const filteredItems = computed(() => {
  const items = adminBillsStore.bills.filter((i) => {
    if (
...
onMounted(async () => {
  await adminBillsStore.refreshBills()
})

const doShowEditItemModal = ref(false)
const doShowViewItemModal = ref(false)
const itemToEdit = ref<AdminBill | null>(null)
const errorMsg = ref('')

const showEditItemModal = (item: AdminBill) => {
  itemToEdit.value = item
  errorMsg.value = ''
  doShowEditItemModal.value = true
}

const showViewItemModal = (item: AdminBill) => {
  itemToEdit.value = item
