<template>
  ...
  <VaCard>
    <VaCardContent>
      ...
      <PlasmixMetabolismTable
        :items="items"
        :loading="adminOmicsStore.plasmixMetabolismLoading"
        :prevent-download="adminOmicsStore.plasmixMetabolismPreventDownload"
  ...
  <VaModal
    ...
    close-button
  >
    <h1 class="va-h5">{{ `${t('plasmix_metabolism.view_data')}: ${itemToEdit.id}` }}</h1>
    <ViewPlasmixMetabolismForm :item="itemToEdit" @close="ok" />
  </VaModal>
  ...
  <VaModal
    ...
    close-button
  >
    <h1 class="va-h5">{{ `${t('plasmix_metabolism.edit_data')}: ${itemToEdit.id}` }}</h1>
...
import { Filters, Pagination, getAdminPlasmixMetabolismData } from '../../../data/pages/omics'
import { useAdminOmicsStore } from '../../../stores/admin-omics'
import { download } from '../../omics/composables/useOmics'
import { AdminPlasmixMetabolismItem, RawDataType } from './types'
...
import ViewPlasmixMetabolismForm from './widgets/ViewPlasmixMetabolismForm.vue'
...
const filteredItems = computed(() => {
  const items = adminOmicsStore.plasmixMetabolismItems.filter((i) => {
    if (
...
watch(
  [() => adminOmicsStore.plasmixMetabolismItems.length],
  () => {
    const userMap = new Map<number, string>()
    adminOmicsStore.plasmixMetabolismItems.forEach((item) => {
...
onMounted(async () => {
  await adminOmicsStore.refreshPlasmixMetabolism()
})

const doShowEditItemModal = ref(false)
const doShowViewItemModal = ref(false)
const itemToEdit = ref<AdminPlasmixMetabolismItem>(null as unknown as AdminPlasmixMetabolismItem)
const errorMsg = ref('')

const showEditItemModal = (item: AdminPlasmixMetabolismItem) => {
  itemToEdit.value = item
  errorMsg.value = ''
  doShowEditItemModal.value = true
}

const showViewItemModal = (item: AdminPlasmixMetabolismItem) => {
  itemToEdit.value = item
...
const onRowDataDownload = async (item: AdminPlasmixMetabolismItem, type: RawDataType) => {
  ...
  if (agreed) {
    const data = await getAdminPlasmixMetabolismData(item)
    if (type === 'meta') {
...
const onReportDownload = async (item: AdminPlasmixMetabolismItem) => {
  const agreed = await confirm({
  ...
  if (agreed) {
    const url = await adminOmicsStore.getPlasmixMetabolismReportUrl(item)
    download(url, 'Quartet_Plasmix_Metabolism_QC_Report.pdf')
  }
}

const onItemSaved = async (item: AdminPlasmixMetabolismItem, ok: () => void) => {
  errorMsg.value = ''
  const agreed = await confirm({
  ...
  if (agreed) {
    try {
      await adminOmicsStore.updatePlasmixMetabolismNotes(item)
      ok()
    } catch (e) {
