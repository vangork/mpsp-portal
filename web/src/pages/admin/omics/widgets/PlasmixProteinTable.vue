...
const columns = defineVaDataTableColumns([
  ...
  { label: t('plasmix_protein.name'), key: 'name', sortable: true },
  { label: t('plasmix_protein.meta_file'), key: 'meta_file' },
  { label: t('plasmix_protein.exp_file'), key: 'exp_file' },
  { label: t('plasmix_protein.determinated_at'), key: 'determinated_at', sortable: true },
  { label: t('omics.qc_result'), key: 'qc_result', sortable: true },
  { label: 'features', key: 'features', sortable: false },
  { label: 'recall', key: 'recall', sortable: false },
  { label: 'snr', key: 'snr', sortable: false },
  { label: 'rc', key: 'rc', sortable: false },
...
const props = defineProps({
  items: {
    type: Array as PropType<AdminPlasmixProteinItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'download-item', item: AdminPlasmixProteinItem, type: RawDataType): void
  (event: 'edit-item', item: AdminPlasmixProteinItem): void
  (event: 'view-item', item: AdminPlasmixProteinItem): void
  (event: 'download-report', item: AdminPlasmixProteinItem): void
}>()
  ...
  >
    ...
    <template #cell(meta_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminPlasmixProteinItem, 'meta')"
        />
      </div>
    </template>
    ...
    <template #cell(exp_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminPlasmixProteinItem, 'exp')"
        />
      </div>
    </template>
    ...
    <template #cell(qc_result)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        <VaBadge
          v-if="
            rowData.identified_proteins &&
            rowData.recall &&
            rowData.snr &&
            rowData.rc &&
            rowData.identified_proteins >= 1000 &&
            rowData.snr >= 5
          "
          text="pass"
          color="success"
    ...
    <template #cell(features)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        {{ rowData.identified_proteins }}
      </div>
    </template>

    <template #cell(recall)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        {{ roundTo(rowData.recall, 3) }}
      </div>
    </template>
    ...
    <template #cell(rc)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        {{ roundTo(rowData.rc, 3) }}
      </div>
    </template>

    <template #cell(sample_number)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        {{ rowData.m + rowData.f + rowData.p + rowData.x + rowData.y }}
      </div>
    </template>
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="emit('view-item', rowData as AdminPlasmixProteinItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('download-report', rowData as AdminPlasmixProteinItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('edit-item', rowData as AdminPlasmixProteinItem)"
        />
      </div>
    </template>
  </VaDataTable>
