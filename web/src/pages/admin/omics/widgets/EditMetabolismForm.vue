...
import { AdminMetabolismItem } from '../types'

const { t } = useI18n()

const props = defineProps({
  item: {
    type: Object as PropType<AdminMetabolismItem>,
    required: true,
  },
})

const newItem = ref<AdminMetabolismItem>({ ...props.item })
  ...
  <VaForm class="flex-col justify-start items-start gap-4 inline-flex w-full">
    <div class="self-stretch flex-col justify-start items-start gap-4 flex">
      <div class="flex gap-4 flex-col sm:flex-row w-full">
        <VaInput :model-value="newItem.name" :label="t('metabolism.name')" class="w-full sm:w-1/2" disabled />
      </div>
