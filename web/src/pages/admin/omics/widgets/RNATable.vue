...
const columns = defineVaDataTableColumns([
  ...
  { label: t('rna.name'), key: 'name', sortable: true },
  { label: t('rna.meta_file'), key: 'meta_file' },
  { label: t('rna.exp_file'), key: 'exp_file' },
  { label: t('rna.count_file'), key: 'count_file' },
  { label: t('rna.determinated_at'), key: 'determinated_at', sortable: true },
...
const props = defineProps({
  items: {
    type: Array as PropType<AdminRNAItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'download-item', item: AdminRNAItem, type: RawDataType): void
  (event: 'edit-item', item: AdminRNAItem): void
  (event: 'view-item', item: AdminRNAItem): void
  (event: 'download-report', item: AdminRNAItem): void
}>()
    ...
    <template #cell(meta_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminRNAItem, 'meta')"
        />
      </div>
    </template>
    ...
    <template #cell(exp_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminRNAItem, 'exp')"
        />
      </div>
    </template>

    <template #cell(count_file)="{ rowData }">
      <div>
        {{ rowData.count_file }}
        <VaButton
          preset="primary"
          size="small"
          icon="mso-download"
          aria-label="Download Count file"
          @click="emit('download-item', rowData as AdminRNAItem, 'count')"
        />
      </div>
    </template>
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="emit('view-item', rowData as AdminRNAItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('download-report', rowData as AdminRNAItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('edit-item', rowData as AdminRNAItem)"
        />
      </div>
    </template>
  </VaDataTable>
