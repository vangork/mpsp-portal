...
const columns = defineVaDataTableColumns([
  ...
  { label: t('plasmix_metabolism.name'), key: 'name', sortable: true },
  { label: t('plasmix_metabolism.meta_file'), key: 'meta_file' },
  { label: t('plasmix_metabolism.exp_file'), key: 'exp_file' },
  { label: t('plasmix_metabolism.determinated_at'), key: 'determinated_at', sortable: true },
...
const props = defineProps({
  items: {
    type: Array as PropType<AdminPlasmixMetabolismItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'download-item', item: AdminPlasmixMetabolismItem, type: RawDataType): void
  (event: 'edit-item', item: AdminPlasmixMetabolismItem): void
  (event: 'view-item', item: AdminPlasmixMetabolismItem): void
  (event: 'download-report', item: AdminPlasmixMetabolismItem): void
}>()
  ...
  >
    ...
    <template #cell(meta_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminPlasmixMetabolismItem, 'meta')"
        />
      </div>
    </template>
    ...
    <template #cell(exp_file)="{ rowData }">
      <div>
        ...
        <VaButton
          ...
          @click="emit('download-item', rowData as AdminPlasmixMetabolismItem, 'exp')"
        />
      </div>
    </template>
    ...
    <template #cell(qc_result)="{ rowData }">
      <div class="ellipsis max-w-[100px]">
        <VaBadge v-if="rowData.snr && rowData.pcc && rowData.snr >= 5" text="pass" color="success" />
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="emit('view-item', rowData as AdminPlasmixMetabolismItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('download-report', rowData as AdminPlasmixMetabolismItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('edit-item', rowData as AdminPlasmixMetabolismItem)"
        />
      </div>
    </template>
  </VaDataTable>
