...
import { MetabolismItem } from '../types'
...
const columns = defineVaDataTableColumns([
  ...
  { label: t('metabolism.ms_instrument'), key: 'ms_instrument', sortable: true },
...
const props = defineProps({
  items: {
    type: Array as PropType<MetabolismItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'edit-item', item: MetabolismItem): void
  (event: 'download-item', item: MetabolismItem): void
}>()
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="$emit('edit-item', rowData as MetabolismItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="onItemDownload(rowData as MetabolismItem)"
        />
      </div>
    </template>
  </VaDataTable>
