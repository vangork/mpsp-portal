...
import { PlasmixMetabolismItem } from '../types'
...
const columns = defineVaDataTableColumns([
  ...
  { label: t('plasmix_metabolism.ms_instrument'), key: 'ms_instrument', sortable: true },
...
const props = defineProps({
  items: {
    type: Array as PropType<PlasmixMetabolismItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'edit-item', item: PlasmixMetabolismItem): void
  (event: 'download-item', item: PlasmixMetabolismItem): void
}>()
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="$emit('edit-item', rowData as PlasmixMetabolismItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="onItemDownload(rowData as PlasmixMetabolismItem)"
        />
      </div>
    </template>
  </VaDataTable>
