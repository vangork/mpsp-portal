...
const columns = defineVaDataTableColumns([
  ...
  { label: t('rna.platform_model'), key: 'platform_model', sortable: true },
...
const props = defineProps({
  items: {
    type: Array as PropType<RNAItem[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: 'edit-item', item: RNAItem): void
  (event: 'download-item', item: RNAItem): void
}>()
...
<template>
  ...
  >
    ...
    <template #cell(platform_model)="{ rowData }">
      <div class="ellipsis max-w-[230px]">
        {{ rowData.platform_model }}
      </div>
    </template>
    ...
    <template #cell(count_file)="{ rowData }">
      <div>
        {{ recoverFileName(rowData.count_file) }}
      </div>
    </template>
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="$emit('edit-item', rowData as RNAItem)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="onItemDownload(rowData as RNAItem)"
        />
      </div>
    </template>
  </VaDataTable>
